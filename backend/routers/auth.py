...
from backend.encryption.security import get_password_hash, verify_password, create_access_token, get_current_user
...
def get_profile(current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {"id": user.id, "email": user.email, "registered_at": user.created_at}
